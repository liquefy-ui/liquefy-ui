...
Rebuild the lens optics so the rim can no longer fold, and add the lean
...
New: `LiquidGlass`, the material with its optics as props; `elasticity`, a lean
toward a pointer that has not arrived yet, driven through the existing springs
so it overshoots rather than glides; and `glow`, `ripple`, `shimmer` and
`sparkle` as separate switches, so the optics can be had without the ornaments.
The lit rim is now two blended rings whose bright quarter follows the pointer.
