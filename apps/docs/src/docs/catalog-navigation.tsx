...
<LiquidDrawer onOpenChange={setOpen} open={open} title="Filters">
  …
    ...
    description: 'A sliding side panel built on Base UI, with a trapped focus ring, scroll lock and Escape handling. Slides from the left, right, or bottom edge, and a finger flicked towards that edge dismisses it — unless the content under the finger can still scroll that way, or the flick belongs to a popup the panel opened.',
    ...
    props: [
      ...
      { defaultValue: 'true', description: 'Dismiss the panel when it is flicked towards its own edge. Touch and pen only.', name: 'swipeToClose', type: 'boolean' },
